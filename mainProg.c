...
//Load input image    
  ...
  if(argc < 3)
  {
    ...
    printf("Usage : ./a.out <input_image_file> <Interpolate Ratio> <thread no.>");
    exit(0);
  }
...
img2 = NearestNeighborInterpolation(img, Ratio,thread_count);
...
img3 = bilinearInterpolation(img, Ratio, thread_count);
